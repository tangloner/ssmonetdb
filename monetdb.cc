...
{
	dbh = mapi_connect("localhost", 50000, "monetdb", "monetdb", "sql", conninfo.c_str()); 
	if (mapi_error(dbh))
        {
                if (hdl != NULL)
                {
...
schema_monetdb::schema_monetdb(std::string &conninfo):monetdb_connection(conninfo)
{
	...
	cerr << "Loading tables from database: " <<conninfo;
//	string qry = "select t.name, s.name, t.system, t.type from sys.tables t,  sys.schemas s where t.schema_id=s.id and t.system=false";
	...
	// TODO: confirm with Martine or Stefan about column constraints in MonetDB	
	cerr << " done." << endl;
...
{
	hdl = mapi_query(dbh,"CALL sys.settimeout(1)");
